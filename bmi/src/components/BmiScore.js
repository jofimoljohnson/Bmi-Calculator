function BmiScore({ bmiNo, bmiName, changeWeight }) {
    ...
    return (
        <div className="text-center shadow rounded p-4">
            ...
            {changeWeight.type === "postitive" && (
                <div className="fs-4">
                    "You need lose<span className="fw-bold">{changeWeight.wight}kg"</span>
                </div>
            )}
            {changeWeight.type === "negative" && (
                <div className="fs-4">
                    "You need gain<span className="fw-bold">{changeWeight.wight}kg"</span>
                </div>
            )}
            {changeWeight.type === "normal" && (
                <div className="fs-4">
                    "You weight is normal,Good for you<span className="fw-bold">{changeWeight.wight}kg"</span>
                </div>
            )}
        </div>
    );
